...
Linearizing these ODEs around a steady-state point simplifies the analysis, enabling the representation of gene interactions through a connectivity matrix \(A\), where \(a_{ij}\) quantifies the influence of gene \(j\) on gene \(i\)\supercite{yeung_reverse_2002, akutsu_identification_1998, liang_reveal_nodate, gardner_inferring_2003}.


\noindent To capture the system's responses to external changes, perturbations (\(b_i\)) are incorporated into the ODE framework, providing a means to simulate environmental or experimental variations\supercite{yeung_reverse_2002, liang_reveal_nodate}.
...
\begin{itemize}
    ...
    \item \textbf{Mutual Information and Boolean Networks:} Algorithms like REVEAL use mutual information to infer regulatory relationships based on input-output state transitions, suitable for binary models of gene activity\supercite{liang_reveal_nodate, akutsu_identification_1998}.
    ...
    When this is infeasible, techniques like ridge regression or sparsity constraints become essential\supercite{gardner_inferring_2003, yeung_reverse_2002}.
...
Arbitrary thresholding not only impacts the detection of biologically relevant interactions but also raises concerns about the method’s capacity to reflect the true complexity of gene regulatory mechanisms\supercite{gardner_reverse-engineering_2005, barabasi_network_2004}.
